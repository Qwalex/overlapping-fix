...
const getCircle = (el)=>{
    ...
    return {
        ...
        height,
        rad: rad1,
        centerX: left - imgClientRect.left - 1 + rad1,
        centerY: top - imgClientRect.top - 1 + rad1
    };
};
const getDistance = (circle1, circle2)=>{
    if (circle1.el === circle2.el) return 0;
    const { centerX: x1 , centerY: y1 ,  } = circle1;
...
const createCircle = ([offsetX, offsetY])=>{
...
window.shiftItem = (el)=>{
    ...
    allCircles.forEach((circle)=>{
        console.log(circle.el, shiftCircle.el);
        console.log(circle.el === shiftCircle.el);
        if (circle.el === shiftCircle.el) return;
        const distance = getDistance(circle, shiftCircle);
        console.log(distance);
        if (distance < 0) {
            const angle = getAngle(circle, shiftCircle);
            const [x, y] = getShiftCoords([
                shiftCircle.centerX,
                shiftCircle.centerY
            ], angle, Math.abs(distance));
...
for(let i = 0; i < 50; i++)setRandomCircle();
