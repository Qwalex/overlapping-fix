...
})({"3VBnf":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "c3f07271215376d7";
...
window.g = (()=>{
    const circle11 = document.getElementById('c1');
    const circle21 = document.getElementById('c2');
    ...
    img.addEventListener('mousemove', (e)=>{
        ...
        if (!availablePosition) document.body.classList.add('can-not-add');
        else document.body.classList.remove('can-not-add');
    });
    img.addEventListener('click', (e)=>{
        console.log('click');
        const availablePosition = checkPosition(e);
        console.log(availablePosition);
        if (!availablePosition) return false;
        console.log(availablePosition);
    ...
    const checkPosition = (e)=>{
        ...
        getAllCircles().forEach((circle)=>{
            ...
            if (distance < 0) {
                ...
                circlePlaceholders.push(createCircle(getShiftCoords([
                    offsetX,
                    offsetY
                ], angle, Math.abs(distance)), true));
    ...
    const getDistance = (circle1, circle2)=>{
        const { centerX: x1 , centerY: y1 , rad ,  } = circle1;
    ...
    const getAllCircles = ()=>{
        return [
            ...document.querySelectorAll('.js-circle')
        ].map((el)=>getCircle(el)
        );
    };
    const createCircle = ([offsetX, offsetY], placeholder = false)=>{
        console.log('createCircle');
        const el = document.createElement('div');
        el.classList.add('circle');
        if (!placeholder) el.classList.add('js-circle');
    ...
    const getOffsetByImage = (e)=>{
        return [
            e.clientX - imgClientRect.left,
            e.clientY - imgClientRect.top
        ];
    };
    ...
    const getShiftCoords = ([x, y], angle, distance)=>{
        ...
        return [
            x + (distance + additionalOffset) * Math.cos(degreesToRadians(angle)),
            y + (distance + additionalOffset) * Math.sin(degreesToRadians(angle))
        ];
    };
    const getAvailablePosition = ()=>{
        ...
        const result = nearPositions.filter(([centerX, centerY])=>{
            ...
            getAllCircles().forEach((circle)=>{
                ...
                if (distance < 0) // console.log(`distance incorrect`);
        ...
        if (result.length === 0) return console.log('empty result !!!!');
        return result;
    };
    setRandomCircle = ()=>{
        createCircle([
...
//# sourceMappingURL=index.215376d7.js.map
